...
const int debounce = 2000; // this may need to reduce for TD5 - there are some scenarios where a signal may only be 2.5ms
...
float calFactor = 1.05;   // decrease to slow down speedometer  (use 1/ratio to get calfactors for speedup)
...
void loop()
{
  ...
  if (currentMicros - previousSpeed < 150000) // halved 2hz slowest moving signal from SLABS should be 454micros peak to peak, so if we are seeing much more than half of this then we must be stationary
  { // 1mph = 2.2hz ~230ms, so anything above this means we are stationary
    ...
    { previousMicros = currentMicros;
      ...
      Serial.println(pulseState); //debug out
      digitalWrite(speedometerPin, pulseState); // output our corrected VSS
      digitalWrite(ledPin, pulseState); // blink the LED
      oneHertz = currentMicros;
    }
  }

  if (currentMicros - oneHertz > 250000) { //halved 2hz if we are stationary output a 1hz signal
    if (pulseState == LOW) {
      pulseState = HIGH;
    } else {
      pulseState = LOW;
